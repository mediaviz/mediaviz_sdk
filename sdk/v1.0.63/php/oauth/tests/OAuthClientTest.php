...
class OAuthClientTest extends TestCase
{
    ...
    public function test_request_invokes_on_refresh_success_with_new_tokens(): void
    {
        $calls = 0;
        $transport = function () use (&$calls): array {
            $calls++;
        ...
        $client->request(
            ...
            'refresh-tok',
            null,
            function (TokenResponse $newTokens) use (&$captured): void {
                $captured[] = $newTokens;
            },
        );

        $this->assertCount(1, $captured);
        $this->assertSame('new-access', $captured[0]->accessToken);
        $this->assertSame('new-refresh', $captured[0]->refreshToken);
    }

    public function test_on_refresh_success_fires_even_when_retry_returns_5xx(): void
    {
        $calls = 0;
        $transport = function () use (&$calls): array {
            $calls++;
            return match ($calls) {
                1 => [401, '{}'],
                2 => [200, $this->tokenBody('new-access', 'new-refresh')],
                3 => [500, json_encode(['error' => 'server_error', 'error_description' => 'oops'])],
            };
        };
        $captured = [];
        $client = new OAuthClient($this->config, $transport);
        ...
        try {
            ...
            $this->fail('Expected OAuthError on retry 500');
        } catch (OAuthError) {
            // expected
        }

        $this->assertCount(1, $captured, 'rotated tokens must reach the caller before retry-failure exception');
        $this->assertSame('new-refresh', $captured[0]->refreshToken);
    }

    public function test_on_refresh_success_not_called_when_no_refresh_occurs(): void
    {
        $client = $this->makeClient([[200, json_encode(['user' => 'alice'])]]);
        $captured = [];
        $client->request(
    ...
    public function test_request_omitting_on_refresh_success_remains_valid(): void
    {
        $calls = 0;
        $transport = function () use (&$calls): array {
            $calls++;
