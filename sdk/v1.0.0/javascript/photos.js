export class Photos {
  ...
  async getTopMiddleBottomProjectPhotosByTableNameSortedByDateNewRanked(tableName, { ascOrDesc } = {}) {
    this._ctx.requireTokens();
  ...
  async getTopMiddleBottomProjectPhotosByTableNameSortedByDateNewRankedKeysetPaginated(tableName, { ascOrDesc, lastId, limit } = {}) {
    this._ctx.requireTokens();
    let path = `/api/v1/photos_ranked/${encodeURIComponent(tableName)}/`;
    const query = new URLSearchParams();
    if (ascOrDesc !== undefined) query.set('asc_or_desc', ascOrDesc);
  ...
  async getTopMiddleBottomProjectPhotosByTableNameByMonthSortedByDateNewRankedKeysetPaginated(tableName, month, year, { ascOrDesc, lastId, limit } = {}) {
    this._ctx.requireTokens();
    let path = `/api/v1/photos/${encodeURIComponent(tableName)}/month/${encodeURIComponent(month)}/year/${encodeURIComponent(year)}/ranked/`;
    const query = new URLSearchParams();
    if (ascOrDesc !== undefined) query.set('asc_or_desc', ascOrDesc);
