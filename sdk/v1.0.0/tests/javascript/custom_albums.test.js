...
describe('Custom_Albums', () => {
  ...
  it('getTopMiddleBottomCustomAlbumById — exists', () => {
    const customAlbums = new CustomAlbums({});
    expect(typeof customAlbums.getTopMiddleBottomCustomAlbumById).toBe('function');
  });
  ...
  it('getTopMiddleBottomCustomAlbumById — HTTP method is GET', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const customAlbums = new CustomAlbums(ctx);
    await customAlbums.getTopMiddleBottomCustomAlbumById(42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
    expect(spy.last_call().method).toBe('GET');
  });
  ...
  it('createProjectCustomAlbum — HTTP method is POST', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const customAlbums = new CustomAlbums(ctx);
    await customAlbums.createProjectCustomAlbum('test_value', 'test_value', 'test_value', ['item1', 'item2'], ['item1', 'item2']);
  ...
  it('createProjectCustomAlbum — path construction', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const customAlbums = new CustomAlbums(ctx);
    await customAlbums.createProjectCustomAlbum('hello world', 'test_value', 'test_value', ['item1', 'item2'], ['item1', 'item2']);
    expect(spy.last_call().path).toContain('/api/v1/custom_album/project/hello%20world');
  });
  ...
  it('updateCustomAlbum — HTTP method is PUT', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const customAlbums = new CustomAlbums(ctx);
    await customAlbums.updateCustomAlbum(42, 'test_value', 'test_value', ['item1', 'item2'], ['item1', 'item2']);
