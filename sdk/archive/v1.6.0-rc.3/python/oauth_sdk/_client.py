...
_CLIENT_CREDENTIALS_KEY = "\x00client_credentials"
...
class OAuthClient:
    ...
    def request(
        ...
        body: dict | None = None,
        on_refresh_success: Optional[Callable[[TokenResponse], None]] = None,
    ) -> AuthenticatedResponse:
        """Makes an authenticated request with 401-intercept-and-retry.

        `on_refresh_success` fires synchronously the moment the rotated tokens
        are received, BEFORE the retry. The server has already deleted the old
        refresh token by then; if the retry raises, the new tokens would
        otherwise be lost in this call frame. Long-lived callers MUST persist
        via this callback — not from the returned value — to stay in sync with
        single-use refresh-token rotation (RFC 6749 §6).
        """
        ...
        if not refresh_token and not self._config.client_secret:
            raise OAuthError.from_response(response.status_code, response.json())

        reauth_key = refresh_token or _CLIENT_CREDENTIALS_KEY
        with self._inflight_lock:
            fut = self._inflight_refreshes.get(reauth_key)
            owner = fut is None
            if owner:
                fut = Future()
                self._inflight_refreshes[reauth_key] = fut
        ...
        if owner:
            try:
                if refresh_token:
                    ...
                    fut.set_result(self.get_client_credentials_token())
            except BaseException as e:
                fut.set_exception(e)
            finally:
                with self._inflight_lock:
                    self._inflight_refreshes.pop(reauth_key, None)

        new_tokens = fut.result()

        if on_refresh_success is not None:
            on_refresh_success(new_tokens)
