...
class PhotoUpload:
    ...
    def upload_photo(
        ...
        if (template or {}).get('blur') is not None:
            _headers['x-blur'] = (template or {}).get('blur')
        if (template or {}).get('colors') is not None:
            _headers['x-colors'] = (template or {}).get('colors')
        if (template or {}).get('face_recognition') is not None:
            _headers['x-face-recognition'] = (template or {}).get('face_recognition')
        if (template or {}).get('image_describe') is not None:
            _headers['x-image-describe'] = (template or {}).get('image_describe')
        if (template or {}).get('image_classification') is not None:
            _headers['x-image-classification'] = (template or {}).get('image_classification')
        if (template or {}).get('image_comparison') is not None:
            _headers['x-image-comparison'] = (template or {}).get('image_comparison')
        ...
        if (template or {}).get('ocr') is not None:
            _headers['x-ocr'] = (template or {}).get('ocr')
        _body = {
