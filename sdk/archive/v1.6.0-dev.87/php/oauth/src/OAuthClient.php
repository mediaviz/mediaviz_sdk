...
class OAuthClient
{
    ...
    /**
     * Rotates the refresh token. The server enforces one-time use with no grace period:
     * concurrent callers with the same refresh_token will race, and all but the first get
     * 400 invalid_grant. PHP's shared-nothing per-request model means in-process dedupe
     * isn't possible across workers — callers running multiple PHP-FPM/CLI processes must
     * serialize refreshes externally (e.g., via a shared token store with flock/APCu/Redis).
     */
     ...
     * Returned TokenResponse has no refresh token.
     */
     ...
     * Long-lived callers MUST persist via this callback — not from the returned value —
     * to stay in sync with single-use refresh-token rotation (RFC 6749 §6).
     *
     * @param ?callable(TokenResponse): void $onRefreshSuccess
     */
