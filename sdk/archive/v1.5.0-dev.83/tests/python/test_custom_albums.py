...
def test_post_create_project_custom_album_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.custom_albums.create_project_custom_album('test_value', name='test_value', description='test_value', photoIdInclusionList=['item1', 'item2'], photoIdRemovalList=['item1', 'item2'])
    assert spy_client.last_call()['method'] == 'POST'

def test_post_create_project_custom_album_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.custom_albums.create_project_custom_album('hello world', name='test_value', description='test_value', photoIdInclusionList=['item1', 'item2'], photoIdRemovalList=['item1', 'item2'])
    assert '/api/v1/custom_album/project/hello%20world' in spy_client.last_call()['url']
...
def test_put_update_custom_album_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.custom_albums.update_custom_album(42, name='test_value', description='test_value', photoIdInclusionList=['item1', 'item2'], photoIdRemovalList=['item1', 'item2'])
