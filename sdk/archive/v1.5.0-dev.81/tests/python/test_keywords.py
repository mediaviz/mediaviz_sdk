...
def test_put_update_keyword_filtering_list_details_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.update_keyword_filtering_list_details(42, name='test_value', projectList=['item1', 'item2'])
    assert spy_client.last_call()['method'] == 'PUT'
...
def test_post_add_projects_to_keyword_filtering_list_exists(mv_client):
    assert callable(getattr(mv_client.keywords, 'add_projects_to_keyword_filtering_list', None))

def test_post_add_projects_to_keyword_filtering_list_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.add_projects_to_keyword_filtering_list(42, 'test_value')
    assert spy_client.last_call()['method'] == 'POST'

def test_post_add_projects_to_keyword_filtering_list_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.add_projects_to_keyword_filtering_list(42, 'test_value')
    assert '/api/v1/keyword/42/projects' in spy_client.last_call()['url']

def test_post_add_projects_to_keyword_filtering_list_query_params(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.add_projects_to_keyword_filtering_list(42, 'test_value')
    url = spy_client.last_call()['url']
    assert 'project_ids=' in url

def test_post_add_company_to_keyword_filtering_list_exists(mv_client):
    assert callable(getattr(mv_client.keywords, 'add_company_to_keyword_filtering_list', None))

def test_post_add_company_to_keyword_filtering_list_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.add_company_to_keyword_filtering_list(42, 42)
    assert spy_client.last_call()['method'] == 'POST'

def test_post_add_company_to_keyword_filtering_list_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.add_company_to_keyword_filtering_list(42, 42)
    assert '/api/v1/keyword/42/company' in spy_client.last_call()['url']

def test_post_add_company_to_keyword_filtering_list_query_params(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.add_company_to_keyword_filtering_list(42, 42)
    url = spy_client.last_call()['url']
    assert 'company_id=' in url
...
def test_delete_remove_company_from_keyword_filtering_list_exists(mv_client):
    assert callable(getattr(mv_client.keywords, 'remove_company_from_keyword_filtering_list', None))

def test_delete_remove_company_from_keyword_filtering_list_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.remove_company_from_keyword_filtering_list(42, 42)
    assert spy_client.last_call()['method'] == 'DELETE'

def test_delete_remove_company_from_keyword_filtering_list_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.remove_company_from_keyword_filtering_list(42, 42)
    assert '/api/v1/keyword/42/company' in spy_client.last_call()['url']

def test_delete_remove_company_from_keyword_filtering_list_query_params(mv_client, spy_client):
    spy_client.reset()
    mv_client.keywords.remove_company_from_keyword_filtering_list(42, 42)
    url = spy_client.last_call()['url']
    assert 'company_id=' in url
