...
def test_put_update_company_photo_url_allowlist_exists(mv_client):
    assert callable(getattr(mv_client.company, 'update_company_photo_url_allowlist', None))

def test_put_update_company_photo_url_allowlist_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.company.update_company_photo_url_allowlist(42, ['item1', 'item2'])
    assert spy_client.last_call()['method'] == 'PUT'

def test_put_update_company_photo_url_allowlist_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.company.update_company_photo_url_allowlist(42, ['item1', 'item2'])
    assert '/api/v1/company/42/photo_url_allowlist/' in spy_client.last_call()['url']

def test_put_update_company_photo_url_allowlist_request_body(mv_client, spy_client):
    spy_client.reset()
    mv_client.company.update_company_photo_url_allowlist(42, ['item1', 'item2'])
    body = spy_client.last_call()['body']
    assert 'photo_url_allowlist' in body
