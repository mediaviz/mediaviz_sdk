...
def test_post_create_project_and_run_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.projects.create_project_and_run('test_value', 42, True, 'test_value', 'test_value', 42, 'test_value', 'test_value', 'test_value', 'test_value')
...
def test_put_update_project_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.projects.update_project('test_value', private=True, type=42, description='test_value', directory='test_value', name='test_value', thumbnail='test_value')
    assert spy_client.last_call()['method'] == 'PUT'

def test_put_update_project_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.projects.update_project('hello world', private=True, type=42, description='test_value', directory='test_value', name='test_value', thumbnail='test_value')
    assert '/api/v1/projects/hello%20world' in spy_client.last_call()['url']
...
def test_get_get_project_process_status_exists(mv_client):
    assert callable(getattr(mv_client.projects, 'get_project_process_status', None))

def test_get_get_project_process_status_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.projects.get_project_process_status('test_value', 'test_value')
    assert spy_client.last_call()['method'] == 'GET'

def test_get_get_project_process_status_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.projects.get_project_process_status('hello world', 'hello world')
    assert '/api/v1/projects/hello%20world/process_status/hello%20world' in spy_client.last_call()['url']
