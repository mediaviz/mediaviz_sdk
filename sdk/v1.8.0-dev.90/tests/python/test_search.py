...
def test_get_recall_project_photos_exists(mv_client):
    assert callable(getattr(mv_client.search, 'recall_project_photos', None))

def test_get_recall_project_photos_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.recall_project_photos('test_value', 'test_value', 'test_value', 'test_value', 42)
    assert spy_client.last_call()['method'] == 'GET'

def test_get_recall_project_photos_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.recall_project_photos('hello world', 'test_value', 'test_value', 'test_value', 42)
    assert '/api/v1/search/recall/hello%20world/' in spy_client.last_call()['url']

def test_get_recall_project_photos_query_params(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.recall_project_photos('test_value', 'test_value', 'test_value', 'test_value', 42)
    url = spy_client.last_call()['url']
    assert 'asc_or_desc=' in url
...
def test_post_search_project_photos_natural_language_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language('test_value', 'test_value', 3.14, 3.14, 42, 3.14, 'test_value', 'test_value')
    assert spy_client.last_call()['method'] == 'POST'

def test_post_search_project_photos_natural_language_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language('hello world', 'test_value', 3.14, 3.14, 42, 3.14, 'test_value', 'test_value')
