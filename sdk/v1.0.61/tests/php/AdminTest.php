...
class AdminTest extends TestCase {
    ...
    public function test_post_admin_create_nl_hybrid_pipelines_exists(): void {
        $this->assertTrue(method_exists(Admin::class, 'adminCreateNlHybridPipelines'));
    }

    public function test_post_admin_create_nl_hybrid_pipelines_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminCreateNlHybridPipelines();
        $this->assertSame('POST', $ctx->client->lastCall()['method']);
    }

    public function test_post_admin_create_nl_hybrid_pipelines_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminCreateNlHybridPipelines();
        $this->assertStringContainsString('/api/v1/admin/search/nl_create_hybrid_pipelines', $ctx->client->lastCall()['path']);
    }

    public function test_post_admin_create_nl_hybrid_pipelines_auth_routing(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminCreateNlHybridPipelines();
        $this->assertCount(1, $ctx->client->calls);
    ...
    public function test_post_admin_create_label_vocab_index_exists(): void {
        $this->assertTrue(method_exists(Admin::class, 'adminCreateLabelVocabIndex'));
    }

    public function test_post_admin_create_label_vocab_index_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminCreateLabelVocabIndex();
        $this->assertSame('POST', $ctx->client->lastCall()['method']);
    }

    public function test_post_admin_create_label_vocab_index_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminCreateLabelVocabIndex();
        $this->assertStringContainsString('/api/v1/admin/search/create_label_vocab_index', $ctx->client->lastCall()['path']);
    }

    public function test_post_admin_create_label_vocab_index_auth_routing(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminCreateLabelVocabIndex();
        $this->assertCount(1, $ctx->client->calls);
    ...
    public function test_delete_admin_clear_nl_search_cache_exists(): void {
        $this->assertTrue(method_exists(Admin::class, 'adminClearNlSearchCache'));
    }

    public function test_delete_admin_clear_nl_search_cache_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminClearNlSearchCache();
        $this->assertSame('DELETE', $ctx->client->lastCall()['method']);
    }

    public function test_delete_admin_clear_nl_search_cache_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminClearNlSearchCache();
        $this->assertStringContainsString('/api/v1/admin/clear_nl_search_cache/', $ctx->client->lastCall()['path']);
    }

    public function test_delete_admin_clear_nl_search_cache_auth_routing(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminClearNlSearchCache();
        $this->assertCount(1, $ctx->client->calls);
    ...
    public function test_delete_admin_delete_company_nlp_indexes_exists(): void {
        $this->assertTrue(method_exists(Admin::class, 'adminDeleteCompanyNlpIndexes'));
    }

    public function test_delete_admin_delete_company_nlp_indexes_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminDeleteCompanyNlpIndexes('test_value');
        $this->assertSame('DELETE', $ctx->client->lastCall()['method']);
    }

    public function test_delete_admin_delete_company_nlp_indexes_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminDeleteCompanyNlpIndexes('test_value');
        $this->assertStringContainsString('/api/v1/admin/delete_company_nlp_indexes/', $ctx->client->lastCall()['path']);
    }

    public function test_delete_admin_delete_company_nlp_indexes_query_params(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminDeleteCompanyNlpIndexes('test_value');
        $path = $ctx->client->lastCall()['path'];
    ...
    public function test_delete_admin_delete_company_nlp_indexes_auth_routing(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->adminDeleteCompanyNlpIndexes('test_value');
        $this->assertCount(1, $ctx->client->calls);
