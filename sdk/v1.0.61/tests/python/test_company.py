...
def test_get_admin_get_companies_exists(mv_client):
    assert callable(getattr(mv_client.company, 'admin_get_companies', None))

def test_get_admin_get_companies_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.company.admin_get_companies()
    assert spy_client.last_call()['method'] == 'GET'

def test_get_admin_get_companies_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.company.admin_get_companies()
    assert '/api/v1/companies/admin' in spy_client.last_call()['url']
