...
describe('Projects', () => {
  ...
  it('updateProject — HTTP method is PUT', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const projects = new Projects(ctx);
    await projects.updateProject('test_value', true, 42, 'test_value', 'test_value', 'test_value', 'test_value');
  ...
  it('updateProject — path construction', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const projects = new Projects(ctx);
    await projects.updateProject('hello world', true, 42, 'test_value', 'test_value', 'test_value', 'test_value');
    expect(spy.last_call().path).toContain('/api/v1/projects/hello%20world');
  });
  ...
  it('getProjectDataExportUploadStatus — path construction', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const projects = new Projects(ctx);
    await projects.getProjectDataExportUploadStatus('hello world', 'hello world');
    expect(spy.last_call().path).toContain('/api/v1/projects/hello%20world/upload_status/hello%20world');
  });
