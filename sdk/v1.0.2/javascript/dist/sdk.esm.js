...
class Admin {
  ...
  async requestAlbumCuration() {
    this._ctx.requireTokens();
    const path = `/api/v1/admin/request_album_curation`;
    const { data } = await this._ctx.client.request(path, 'POST', this._ctx.accessToken, this._ctx.refreshToken);
    return data;
  }
  ...
  async adminDeleteUserProjects({ userIds } = {}) {
    ...
    const query = new URLSearchParams();
    if (userIds !== undefined) query.set('user_ids', userIds);
    const qs = query.toString();
    if (qs) path += '?' + qs;
...
class CuratedAlbums {
  ...
  async updateCuratedAlbum(albumId, name = undefined, description = undefined, confidenceValue = undefined) {
    this._ctx.requireTokens();
    const path = `/api/v1/curated_album/${encodeURIComponent(albumId)}`;
    const body = stripUndef$6({
      name: name,
...
class Projects {
  ...
  async updateProject(projectId, private_ = undefined, type = undefined, description = undefined, directory = undefined, name = undefined, thumbnail = undefined) {
    this._ctx.requireTokens();
    const path = `/api/v1/projects/${encodeURIComponent(projectId)}`;
    const body = stripUndef$1({
      private: private_,
