...
function requireBase64 () {
	...
	base64 = { base64urlEncode, base64urlDecode, utf8Decode };
	return base64;
}
...
function requireCrypto$1 () {
	...
	crypto$1 = { configureCrypto, resetCrypto, getCrypto };
	return crypto$1;
}
...
function requirePkce () {
	...
	const { base64urlEncode } = requireBase64();
	const { getCrypto } = requireCrypto$1();
...
function requireClient () {
	...
	const { base64urlDecode, utf8Decode } = requireBase64();
	const { OAuthError } = requireErrors();
	...
	class OAuthClient {
	  ...
	  async revokeToken(token, tokenTypeHint) {
	    const params = { token, ...this._clientAuth() };
	  ...
	  decodeAccessToken(accessToken) {
	    ...
	    return JSON.parse(utf8Decode(base64urlDecode(segment)));
	  }

	  /**
	   * Client-authentication params for a token-endpoint request.
	   *
	   * A public client (mobile, SPA) holds no secret, and the authorization server
	   * rejects the request outright if one is present, so the key is omitted
	   * entirely rather than sent empty. Passing it through unconditionally
	   * serializes `undefined` into the form body as that literal string.
	   * @returns {Record<string, string>}
	   */
	  _clientAuth() {
	    const params = { client_id: this._config.clientId };
	    if (this._config.clientSecret) params.client_secret = this._config.clientSecret;
	    return params;
	  }
	}
...
function requireSrc$2 () {
	...
	const { configureCrypto } = requireCrypto$1();

	src$2 = { OAuthClient, OAuthError, OAuthErrorCode, configureCrypto };
	return src$2;
}

var srcExports$2 = requireSrc$2();
var _oauth = /*@__PURE__*/getDefaultExportFromCjs(srcExports$2);

// Auto-generated — do not edit
const { OAuthClient, OAuthError, OAuthErrorCode, configureCrypto } = _oauth;
...
function requireSrc$1 () {
	...
	src$1 = { WebhookConsumer, InMemoryWebhookStore, signWebhookPayload, verifyWebhookSignature, selectResultColumns, WEBHOOK_MODEL_COLUMNS, WEBHOOK_OUTCOME_MODELS };
	return src$1;
}

var srcExports$1 = requireSrc$1();
var _webhooks = /*@__PURE__*/getDefaultExportFromCjs(srcExports$1);
...
function stripUndef$8(o) { const r = {}; for (const k in o) if (o[k] !== undefined) r[k] = o[k]; return r; }
...
class Photos {
  ...
  async updatePhotoInProject(tableName, photoId, { filePath, title, description, tags } = {}) {
    this._ctx.requireTokens();
    const path = `/api/v1/photos_update/${encodeURIComponent(tableName)}/id/${encodeURIComponent(photoId)}`;
...
class Subscription {
  ...
  async createSubscription(projectTableName, targets, callbackUrl = undefined) {
    this._ctx.requireTokens();
...
class MediaViz {
  constructor(config = {}) {
    ...
    this._hosts = {
      ...
      webhookReceiver: config.hosts?.webhookReceiver ?? _env('MEDIAVIZ_WEBHOOK_RECEIVER_URL'),
      ...(config.hosts || {}),
    };
    ...
    this.webhookReceiver = new Webhookreceiver(_ctx);
...
function requireSrc () {
	...
	const { createCryptoProvider } = requireCrypto();
	const { MemoryTokenStore, createSecureTokenStore, createKeychainTokenStore } = requireStorage();
	const { startAuthSession, parseRedirectUrl, AuthSessionError } = requireAuthSession();
	const { createSession } = requireSession();

	src = { createCryptoProvider, MemoryTokenStore, createSecureTokenStore, createKeychainTokenStore, startAuthSession, parseRedirectUrl, AuthSessionError, createSession };
	return src;
}

var srcExports = requireSrc();
var _react_native = /*@__PURE__*/getDefaultExportFromCjs(srcExports);

// Auto-generated — do not edit
const { createCryptoProvider, MemoryTokenStore, createSecureTokenStore, createKeychainTokenStore, startAuthSession, parseRedirectUrl, AuthSessionError, createSession } = _react_native;

export { Admin, AiModelCredits, ApiError, AuthSessionError, Company, CuratedAlbums, CustomAlbums, EmailTokens, Health, InMemoryWebhookStore, Keywords, MediaViz, MemoryTokenStore, NotFoundError, OAuthClient, OAuthError, OAuthErrorCode, OauthAuthorization, OauthLogin, OauthToken, Person, Photos, Photoupload, Projects, RateLimitError, Search, ServerError, Subscription, Users, ValidationError, WEBHOOK_MODEL_COLUMNS, WEBHOOK_OUTCOME_MODELS, WebhookConsumer, Webhookreceiver, configureCrypto, createCryptoProvider, createKeychainTokenStore, createSecureTokenStore, createSession, handleResponse, parseRedirectUrl, selectResultColumns, signWebhookPayload, startAuthSession, verifyWebhookSignature };
