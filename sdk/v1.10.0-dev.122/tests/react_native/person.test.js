...
import { Person } from '../../react_native/person.js';
...
describe('Person', () => {
  ...
  it('removePersonName — exists', () => {
    const person = new Person({});
    expect(typeof person.removePersonName).toBe('function');
  });

  it('removePersonName — HTTP method is PUT', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const person = new Person(ctx);
    await person.removePersonName('test_value', 'test_value');
    expect(spy.last_call().method).toBe('PUT');
  });

  it('removePersonName — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const person = new Person(ctx);
    await person.removePersonName('hello world', 'hello world');
    expect(spy.last_call().path).toContain('/api/v1/person/hello%20world/remove_name/hello%20world');
  });

  it('removePersonName — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const person = new Person(ctx);
    await person.removePersonName('test_value', 'test_value');
  ...
  it('removePerson — exists', () => {
    const person = new Person({});
    expect(typeof person.removePerson).toBe('function');
  });

  it('removePerson — HTTP method is DELETE', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const person = new Person(ctx);
    await person.removePerson('test_value', 'test_value');
    expect(spy.last_call().method).toBe('DELETE');
  });

  it('removePerson — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const person = new Person(ctx);
    await person.removePerson('hello world', 'hello world');
    expect(spy.last_call().path).toContain('/api/v1/person/hello%20world/hello%20world');
  });

  it('removePerson — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const person = new Person(ctx);
    await person.removePerson('test_value', 'test_value');
