...
import { Users } from '../../react_native/users.js';
...
describe('Users', () => {
  ...
  it('createUserAndCompany — HTTP method is POST', async () => {
    ...
    const users = new Users(ctx);
    await users.createUserAndCompany('test_value', 'user@example.com', 'test_value', 42, 'test_value', 'test_value', 'test_value', 42, { inviteToken: 'test_value' });
    expect(spy.last_call().method).toBe('POST');
  });
  ...
  it('createUserAndCompany — query params', async () => {
    const spy = makeSpyFetch();
    globalThis.fetch = spy;
    const ctx = { baseUrl: 'https://api.example.com' };
    const users = new Users(ctx);
    ...
    expect(url).toContain('invite_token=');
  });
