...
def test_post_post_approve_consent_http_method(mv_client, monkeypatch):
    _mc = _MockClient()
    monkeypatch.setattr(httpx, 'Client', lambda *a, **kw: _mc)
    mv_client.o_auth_authorization.post_approve_consent('test_value', 'test_value')
    assert _mc.recorded[0]['method'] == 'POST'

def test_post_post_approve_consent_path(mv_client, monkeypatch):
    _mc = _MockClient()
    monkeypatch.setattr(httpx, 'Client', lambda *a, **kw: _mc)
    mv_client.o_auth_authorization.post_approve_consent('hello world', 'test_value')
...
def test_post_post_deny_consent_http_method(mv_client, monkeypatch):
    _mc = _MockClient()
    monkeypatch.setattr(httpx, 'Client', lambda *a, **kw: _mc)
    mv_client.o_auth_authorization.post_deny_consent('test_value', 'test_value')
    assert _mc.recorded[0]['method'] == 'POST'

def test_post_post_deny_consent_path(mv_client, monkeypatch):
    _mc = _MockClient()
    monkeypatch.setattr(httpx, 'Client', lambda *a, **kw: _mc)
    mv_client.o_auth_authorization.post_deny_consent('hello world', 'test_value')
    assert '/oauth/consent/hello%20world/deny' in _mc.recorded[0]['url']

def test_get_get_switch_user_exists(mv_client):
    assert callable(getattr(mv_client.o_auth_authorization, 'get_switch_user', None))

def test_get_get_switch_user_http_method(mv_client, monkeypatch):
    _mc = _MockClient()
    monkeypatch.setattr(httpx, 'Client', lambda *a, **kw: _mc)
    mv_client.o_auth_authorization.get_switch_user('test_value', 'test_value')
    assert _mc.recorded[0]['method'] == 'GET'

def test_get_get_switch_user_path(mv_client, monkeypatch):
    _mc = _MockClient()
    monkeypatch.setattr(httpx, 'Client', lambda *a, **kw: _mc)
    mv_client.o_auth_authorization.get_switch_user('hello world', 'test_value')
    assert '/oauth/consent/hello%20world/switch-user' in _mc.recorded[0]['url']
