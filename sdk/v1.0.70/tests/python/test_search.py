...
def test_post_search_project_photos_text_exists(mv_client):
    assert callable(getattr(mv_client.search, 'search_project_photos_text', None))

def test_post_search_project_photos_text_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_text('test_value', 'test_value', 42)
    assert spy_client.last_call()['method'] == 'POST'

def test_post_search_project_photos_text_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_text('hello world', 'test_value', 42)
    assert '/api/v1/search/text/hello%20world/' in spy_client.last_call()['url']

def test_post_search_project_photos_text_request_body(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_text('test_value', 'test_value', 42)
...
def test_post_search_project_photos_natural_language_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language('test_value', 'test_value', 42)
    assert spy_client.last_call()['method'] == 'POST'

def test_post_search_project_photos_natural_language_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language('hello world', 'test_value', 42)
    assert '/api/v1/search/nl/hello%20world/' in spy_client.last_call()['url']
...
def test_post_search_project_photos_natural_language_hybrid_exists(mv_client):
    assert callable(getattr(mv_client.search, 'search_project_photos_natural_language_hybrid', None))

def test_post_search_project_photos_natural_language_hybrid_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language_hybrid('test_value', 'test_value', 42, 'test_value', 3.14)
    assert spy_client.last_call()['method'] == 'POST'

def test_post_search_project_photos_natural_language_hybrid_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language_hybrid('hello world', 'test_value', 42, 'test_value', 3.14)
    assert '/api/v1/search/nl_hybrid/hello%20world/' in spy_client.last_call()['url']

def test_post_search_project_photos_natural_language_hybrid_query_params(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language_hybrid('test_value', 'test_value', 42, 'test_value', 3.14)
    url = spy_client.last_call()['url']
    assert 'blend=' in url
...
def test_post_search_project_photos_natural_language_hybrid_request_body(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language_hybrid('test_value', 'test_value', 42, 'test_value', 3.14)
    body = spy_client.last_call()['body']
    assert 'search_text' in body
...
def test_post_search_project_photos_natural_language_auto_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language_auto('test_value', 'test_value', 42, 'test_value', 3.14, 3.14, 42, 3.14, True)
    assert spy_client.last_call()['method'] == 'POST'

def test_post_search_project_photos_natural_language_auto_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.search.search_project_photos_natural_language_auto('hello world', 'test_value', 42, 'test_value', 3.14, 3.14, 42, 3.14, True)
