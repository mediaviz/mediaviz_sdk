...
class Keywords:
    ...
    def add_projects_to_keyword_filtering_list(
        self,
        keyword_list_id: int,
    ...
    def add_company_to_keyword_filtering_list(
        self,
        keyword_list_id: int,
        company_id: int | None = None,
    ) -> dict[str, Any]:
        self._ctx.require_tokens()
        path = '/api/v1/keyword/' + quote(str(keyword_list_id), safe='') + '/company'
        _q: dict[str, Any] = {}
        if company_id is not None:
            _q['company_id'] = company_id
        if _q:
            path += '?' + urlencode(_q, doseq=True)
        return self._ctx.client.request(path, 'POST', self._ctx.access_token, self._ctx.refresh_token).data
    ...
    def remove_company_from_keyword_filtering_list(
        self,
        keyword_list_id: int,
        company_id: int | None = None,
    ) -> dict[str, Any]:
