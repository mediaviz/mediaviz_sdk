...
class EmailTokens {
    ...
    public function requestEmailVerification(?string $email = null): mixed {
    ...
    public function verifyEmail(string $token): mixed {
        $baseUrl = $this->ctx->baseUrl;
        $path = "/api/v1/verify-email/" . rawurlencode((string)$token);
        $ch = curl_init($baseUrl . $path);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
        curl_setopt($ch, CURLOPT_HEADER, true);
    ...
    public function requestPasswordReset(?string $email = null): mixed {
        $baseUrl = $this->ctx->baseUrl;
    ...
    public function deleteUserEmailTokens(int $userId): mixed {
        $this->ctx->requireTokens();
        $path = "/api/v1/admin/email_tokens/by_user/" . rawurlencode((string)$userId);
