...
class Projects:
    ...
    def create_project_and_run(
        ...
        if outcomes is not None:
            _q['outcomes'] = str(outcomes)
        if models is not None:
            _q['models'] = str(models)
        if _q:
            path += '?' + urlencode(_q)
        body = {k: v for k, v in {
            'name': name,
    ...
    def mark_project_upload_complete(
        ...
        if skipped_file_count is not None:
            _q['skipped_file_count'] = str(skipped_file_count)
        if _q:
            path += '?' + urlencode(_q)
    ...
    def get_all_user_projects_admin(self, user_id: int) -> dict[str, Any]:
        self._ctx.require_tokens()
        path = '/api/v1/projects/admin/user/' + quote(str(user_id), safe='')
        return self._ctx.client.request(path, 'GET', self._ctx.access_token, self._ctx.refresh_token).data
    ...
    def update_project_photo_count(
        ...
        if files_failed_count is not None:
            _q['files_failed_count'] = str(files_failed_count)
        if _q:
            path += '?' + urlencode(_q)
    ...
    def request_project_admin_export(self, project_table_name: str) -> dict[str, Any]:
        self._ctx.require_tokens()
        path = '/api/v1/projects_admin_export/' + quote(str(project_table_name), safe='')
        return self._ctx.client.request(path, 'GET', self._ctx.access_token, self._ctx.refresh_token).data

    def get_project_data_export_upload_status(
        self,
        project_table_name: str,
        model_name: str,
    ) -> dict[str, Any]:
        self._ctx.require_tokens()
        path = '/api/v1/projects/' + quote(str(project_table_name), safe='') + '/upload_status/' + quote(str(model_name), safe='')
        return self._ctx.client.request(path, 'GET', self._ctx.access_token, self._ctx.refresh_token).data
