...
class OAuthAuthorization:
    ...
    def authorize(
        ...
        if response_type is not None:
            _q['response_type'] = str(response_type)
        if client_id is not None:
            _q['client_id'] = str(client_id)
        if redirect_uri is not None:
            _q['redirect_uri'] = str(redirect_uri)
        if state is not None:
            _q['state'] = str(state)
        if code_challenge is not None:
            _q['code_challenge'] = str(code_challenge)
        if code_challenge_method is not None:
            _q['code_challenge_method'] = str(code_challenge_method)
        if _q:
            path += '?' + urlencode(_q)
    ...
    def post_approve_consent(self, session_id: str) -> dict[str, Any]:
        path = '/oauth/consent/' + quote(str(session_id), safe='') + '/approve'
        with httpx.Client() as _client:
            _resp = _client.request('POST', self._ctx.base_url + path)
        return handle_response(_resp.text, _resp.status_code, dict(_resp.headers))

    def post_deny_consent(self, session_id: str) -> dict[str, Any]:
