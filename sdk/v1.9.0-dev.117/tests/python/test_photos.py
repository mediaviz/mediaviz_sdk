...
def test_put_update_photo_in_project_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.photos.update_photo_in_project('test_value', 42, filePath='test_value', title='test_value', description='test_value', tags=['item1', 'item2'])
    assert spy_client.last_call()['method'] == 'PUT'

def test_put_update_photo_in_project_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.photos.update_photo_in_project('hello world', 42, filePath='test_value', title='test_value', description='test_value', tags=['item1', 'item2'])
    assert '/api/v1/photos_update/hello%20world/id/42' in spy_client.last_call()['url']

def test_put_update_photo_in_project_request_body(mv_client, spy_client):
    spy_client.reset()
    mv_client.photos.update_photo_in_project('test_value', 42, filePath='test_value', title='test_value', description='test_value', tags=['item1', 'item2'])
