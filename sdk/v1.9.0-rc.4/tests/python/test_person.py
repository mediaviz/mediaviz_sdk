...
def test_put_remove_person_name_exists(mv_client):
    assert callable(getattr(mv_client.person, 'remove_person_name', None))

def test_put_remove_person_name_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.person.remove_person_name('test_value', 'test_value')
    assert spy_client.last_call()['method'] == 'PUT'

def test_put_remove_person_name_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.person.remove_person_name('hello world', 'hello world')
    assert '/api/v1/person/hello%20world/remove_name/hello%20world' in spy_client.last_call()['url']
...
def test_get_get_photo_ids_by_persons_exists(mv_client):
    assert callable(getattr(mv_client.person, 'get_photo_ids_by_persons', None))

def test_get_get_photo_ids_by_persons_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.person.get_photo_ids_by_persons('test_value', 'test_value')
    assert spy_client.last_call()['method'] == 'GET'

def test_get_get_photo_ids_by_persons_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.person.get_photo_ids_by_persons('hello world', 'test_value')
    assert '/api/v1/person/hello%20world/photos' in spy_client.last_call()['url']

def test_get_get_photo_ids_by_persons_query_params(mv_client, spy_client):
    spy_client.reset()
    mv_client.person.get_photo_ids_by_persons('test_value', 'test_value')
    url = spy_client.last_call()['url']
    assert 'person_id=' in url
...
def test_delete_remove_person_exists(mv_client):
    assert callable(getattr(mv_client.person, 'remove_person', None))

def test_delete_remove_person_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.person.remove_person('test_value', 'test_value')
    assert spy_client.last_call()['method'] == 'DELETE'

def test_delete_remove_person_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.person.remove_person('hello world', 'hello world')
    assert '/api/v1/person/hello%20world/hello%20world' in spy_client.last_call()['url']
