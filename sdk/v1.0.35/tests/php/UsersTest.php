...
class UsersTest extends TestCase {
    ...
    public function test_get_get_all_users_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Users($ctx);
        $obj->getAllUsers('hello world', 'test_value', 'test_value');
        $this->assertStringContainsString('/api/v1/users/admin/sort/hello%20world/', $ctx->client->lastCall()['path']);
    }
    ...
    public function test_put_update_user_by_admin_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Users($ctx);
        $obj->updateUserByAdmin(42, 'test_value', 'user@example.com', 'test_value', 42, 42, 'test_value', 'test_value', 'test_value', '2024-01-01');
        $this->assertSame('PUT', $ctx->client->lastCall()['method']);
    }
