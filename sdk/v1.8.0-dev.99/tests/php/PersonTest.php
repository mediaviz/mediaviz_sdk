...
class PersonTest extends TestCase {
    ...
    public function test_put_remove_person_name_exists(): void {
        $this->assertTrue(method_exists(Person::class, 'removePersonName'));
    }

    public function test_put_remove_person_name_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->removePersonName('test_value', 'test_value');
        $this->assertSame('PUT', $ctx->client->lastCall()['method']);
    }

    public function test_put_remove_person_name_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->removePersonName('hello world', 'hello world');
        $this->assertStringContainsString('/api/v1/person/hello%20world/remove_name/hello%20world', $ctx->client->lastCall()['path']);
    }

    public function test_put_remove_person_name_auth_routing(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->removePersonName('test_value', 'test_value');
        $this->assertCount(1, $ctx->client->calls);
    ...
    public function test_get_get_photo_ids_by_persons_exists(): void {
        $this->assertTrue(method_exists(Person::class, 'getPhotoIdsByPersons'));
    }

    public function test_get_get_photo_ids_by_persons_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->getPhotoIdsByPersons('test_value', 'test_value');
        $this->assertSame('GET', $ctx->client->lastCall()['method']);
    }

    public function test_get_get_photo_ids_by_persons_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->getPhotoIdsByPersons('hello world', 'test_value');
        $this->assertStringContainsString('/api/v1/person/hello%20world/photos', $ctx->client->lastCall()['path']);
    }

    public function test_get_get_photo_ids_by_persons_query_params(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->getPhotoIdsByPersons('test_value', 'test_value');
        $path = $ctx->client->lastCall()['path'];
        $this->assertStringContainsString('person_id=', $path);
    }

    public function test_get_get_photo_ids_by_persons_auth_routing(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->getPhotoIdsByPersons('test_value', 'test_value');
        $this->assertCount(1, $ctx->client->calls);
    ...
    public function test_delete_remove_person_exists(): void {
        $this->assertTrue(method_exists(Person::class, 'removePerson'));
    }

    public function test_delete_remove_person_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->removePerson('test_value', 'test_value');
        $this->assertSame('DELETE', $ctx->client->lastCall()['method']);
    }

    public function test_delete_remove_person_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->removePerson('hello world', 'hello world');
        $this->assertStringContainsString('/api/v1/person/hello%20world/hello%20world', $ctx->client->lastCall()['path']);
    }

    public function test_delete_remove_person_auth_routing(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Person($ctx);
        $obj->removePerson('test_value', 'test_value');
        $this->assertCount(1, $ctx->client->calls);
