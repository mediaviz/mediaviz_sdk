...
def test_post_request_album_curation_exists(mv_client):
    assert callable(getattr(mv_client.admin, 'request_album_curation', None))

def test_post_request_album_curation_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.admin.request_album_curation()
    assert spy_client.last_call()['method'] == 'POST'

def test_post_request_album_curation_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.admin.request_album_curation()
    assert '/api/v1/admin/request_album_curation' in spy_client.last_call()['url']
