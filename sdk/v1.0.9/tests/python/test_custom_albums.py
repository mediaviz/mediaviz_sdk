...
def test_get_get_top_middle_bottom_custom_album_by_id_exists(mv_client):
    assert callable(getattr(mv_client.custom_albums, 'get_top_middle_bottom_custom_album_by_id', None))

def test_get_get_top_middle_bottom_custom_album_by_id_http_method(mv_client, spy_client):
    spy_client.reset()
    mv_client.custom_albums.get_top_middle_bottom_custom_album_by_id(42, 'test_value', 'test_value', 'test_value')
    assert spy_client.last_call()['method'] == 'GET'

def test_get_get_top_middle_bottom_custom_album_by_id_path(mv_client, spy_client):
    spy_client.reset()
    mv_client.custom_albums.get_top_middle_bottom_custom_album_by_id(42, 'test_value', 'test_value', 'test_value')
    assert '/api/v1/custom_album/photos/ranked/42/' in spy_client.last_call()['url']

def test_get_get_top_middle_bottom_custom_album_by_id_query_params(mv_client, spy_client):
    spy_client.reset()
    mv_client.custom_albums.get_top_middle_bottom_custom_album_by_id(42, 'test_value', 'test_value', 'test_value')
    url = spy_client.last_call()['url']
    assert 'asc_or_desc=' in url
