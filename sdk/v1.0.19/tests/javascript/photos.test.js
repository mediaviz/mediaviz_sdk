...
describe('Photos', () => {
  ...
  it('getRankedProjectPhotosByTableName — HTTP method is GET', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getRankedProjectPhotosByTableName('test_value', { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
    expect(spy.last_call().method).toBe('GET');
  });
  ...
  it('getRankedProjectPhotosByTableName — path construction', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getRankedProjectPhotosByTableName('hello world', { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
  ...
  it('getProjectPhotoIdsByMonth — HTTP method is GET', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getProjectPhotoIdsByMonth('test_value', 42, 42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
    expect(spy.last_call().method).toBe('GET');
  });
  ...
  it('getProjectPhotoIdsByMonth — path construction', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getProjectPhotoIdsByMonth('hello world', 42, 42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
  ...
  it('getRankedProjectPhotosByTableNameByMonth — exists', () => {
    const photos = new Photos({});
    expect(typeof photos.getRankedProjectPhotosByTableNameByMonth).toBe('function');
  });

  it('getRankedProjectPhotosByTableNameByMonth — HTTP method is GET', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getRankedProjectPhotosByTableNameByMonth('test_value', 42, 42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
    expect(spy.last_call().method).toBe('GET');
  });

  it('getRankedProjectPhotosByTableNameByMonth — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getRankedProjectPhotosByTableNameByMonth('hello world', 42, 42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
    expect(spy.last_call().path).toContain('/api/v1/photos/hello%20world/month/42/year/42/ranked/');
  });

  it('getRankedProjectPhotosByTableNameByMonth — query params', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getRankedProjectPhotosByTableNameByMonth('test_value', 42, 42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
  ...
  it('getRankedProjectPhotosByTableNameByMonth — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getRankedProjectPhotosByTableNameByMonth('test_value', 42, 42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
  ...
  it('getProjectPhotoIdsNoDateTaken — exists', () => {
    const photos = new Photos({});
    expect(typeof photos.getProjectPhotoIdsNoDateTaken).toBe('function');
  });
  ...
  it('getProjectPhotoIdsNoDateTaken — HTTP method is GET', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getProjectPhotoIdsNoDateTaken('test_value', { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
    expect(spy.last_call().method).toBe('GET');
  });
  ...
  it('getProjectPhotoIdsNoDateTaken — path construction', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getProjectPhotoIdsNoDateTaken('hello world', { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
    expect(spy.last_call().path).toContain('/api/v1/photos/hello%20world/date_taken/none/');
  });

  it('getProjectPhotoIdsNoDateTaken — query params', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getProjectPhotoIdsNoDateTaken('test_value', { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
  ...
  it('getRankedProjectPhotosByTableNameNoDateTaken — exists', () => {
    const photos = new Photos({});
    expect(typeof photos.getRankedProjectPhotosByTableNameNoDateTaken).toBe('function');
  });

  it('getRankedProjectPhotosByTableNameNoDateTaken — HTTP method is GET', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getRankedProjectPhotosByTableNameNoDateTaken('test_value', { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
    expect(spy.last_call().method).toBe('GET');
  });

  it('getRankedProjectPhotosByTableNameNoDateTaken — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
  ...
  it('getRankedProjectPhotosByTableNameNoDateTaken — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const photos = new Photos(ctx);
    await photos.getRankedProjectPhotosByTableNameNoDateTaken('test_value', { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value' });
