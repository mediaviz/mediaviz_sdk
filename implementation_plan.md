...
- **Pre-release versioning**: `emit_pyproject_toml` appends a PEP 440 suffix from `_PRERELEASE_SUFFIX` (keyed on `gen.prerelease`, set by `generate.py --prerelease dev|rc`): `dev` → `.dev0`, `rc` → `rc0`, `None` → final. Constant suffix; uniqueness comes from the auto-bumped version base. Maps dev/qa/main onto one `mediaviz-sdk` PyPI package (PyPI has no dist-tags). See `spec.md` → "PyPI — `mediaviz-sdk`".
