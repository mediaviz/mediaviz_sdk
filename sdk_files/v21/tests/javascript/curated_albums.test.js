...
describe('Curated_Albums', () => {
  ...
  it('getAllProjectCuratedAlbums — path construction', async () => {
    const spy = new SpyOAuthClient();
    await getAllProjectCuratedAlbums(spy, 'access_token', 'refresh_token', 'test_value');
    expect(spy.last_call().path).toContain('/api/v1/curated_album/project/test_value');
  });
  ...
  it('getCuratedAlbumPhotos — HTTP method is GET', async () => {
    const spy = new SpyOAuthClient();
    await getCuratedAlbumPhotos(spy, 'access_token', 'refresh_token', 42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value', confidenceValue: 'test_value' });
    expect(spy.last_call().method).toBe('GET');
  });
  ...
  it('getCuratedAlbumPhotosRanked — HTTP method is GET', async () => {
    const spy = new SpyOAuthClient();
    await getCuratedAlbumPhotosRanked(spy, 'access_token', 'refresh_token', 42, { ascOrDesc: 'test_value', lastId: 'test_value', limit: 'test_value', confidenceValue: 'test_value' });
    expect(spy.last_call().method).toBe('GET');
  });
