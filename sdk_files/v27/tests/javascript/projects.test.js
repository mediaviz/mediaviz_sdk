...
import { getProjectDataExportUploadStatus, requestProjectExport, getUserProjects, getProjectByDirectory, getProjectById, createProjectAndRun, updateProject, updateProjectPhotoCount, deleteProject, addProjectEvent, updateProjectCreateUploadReport } from '../../javascript/projects.js';
...
describe('Projects', () => {
  ...
  it('getUserProjects — HTTP method is GET', async () => {
    const spy = new SpyOAuthClient();
    await getUserProjects(spy, 'access_token', 'refresh_token');
    expect(spy.last_call().method).toBe('GET');
  });
  ...
  it('getProjectByDirectory — HTTP method is GET', async () => {
    const spy = new SpyOAuthClient();
    await getProjectByDirectory(spy, 'access_token', 'refresh_token', 'test_value');
    expect(spy.last_call().method).toBe('GET');
  });
