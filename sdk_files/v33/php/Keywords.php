...
class Keywords {
    ...
    public function updateKeywordFilteringListLabels(mixed $keywordListId, array $body): mixed {
        $this->ctx->requireTokens();
        $path = "/api/v1/keyword/" . rawurlencode((string)$keywordListId);
        return $this->ctx->client->request($path, 'PUT', $this->ctx->accessToken, $this->ctx->refreshToken, $body)->data;
    }

    public function updateKeywordFilteringListDetails(mixed $keywordListId, array $body): mixed {
        $this->ctx->requireTokens();
        $path = "/api/v1/keyword/details/" . rawurlencode((string)$keywordListId);
        return $this->ctx->client->request($path, 'PUT', $this->ctx->accessToken, $this->ctx->refreshToken, $body)->data;
    }

    public function addProjectToKeywordFilteringList(mixed $keywordListId, mixed $projectIds = null): mixed {
        $this->ctx->requireTokens();
    ...
    public function removeProjectsFromKeywordFilteringList(mixed $keywordListId, mixed $projectIds = null): mixed {
        $this->ctx->requireTokens();
        $path = "/api/v1/keyword/" . rawurlencode((string)$keywordListId) . "/projects";
        $query = [];
        if ($projectIds !== null) $query['project_ids'] = $projectIds;
