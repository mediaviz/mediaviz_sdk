...
class AdminTest extends TestCase {
    ...
    public function test_get_get_category_labels_path(): void {
        ...
        $obj = new Admin($ctx);
        $obj->getCategoryLabels('test_value');
        $this->assertStringContainsString('/api/v1/admin/category_labels/test_value', $ctx->client->lastCall()['path']);
    }
    ...
    public function test_get_get_keyword_groups_labels_by_keyword_group_path(): void {
        ...
        $obj = new Admin($ctx);
        $obj->getKeywordGroupsLabelsByKeywordGroup('test_value', 'test_value');
        $this->assertStringContainsString('/api/v1/admin/keyword_group/test_value/', $ctx->client->lastCall()['path']);
    }
    ...
    public function test_post_request_album_curation_exists(): void {
        $this->assertTrue(method_exists(Admin::class, 'requestAlbumCuration'));
    }

    public function test_post_request_album_curation_http_method(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->requestAlbumCuration();
        $this->assertSame('POST', $ctx->client->lastCall()['method']);
    }

    public function test_post_request_album_curation_path(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->requestAlbumCuration();
        $this->assertStringContainsString('/api/v1/admin/request_album_curation', $ctx->client->lastCall()['path']);
    }

    public function test_post_request_album_curation_auth_routing(): void {
        $ctx = new \OAuthSdk\SpyAuthContext();
        $obj = new Admin($ctx);
        $obj->requestAlbumCuration();
        $this->assertCount(1, $ctx->client->calls);
