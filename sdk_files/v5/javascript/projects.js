...
export function createProjectOutcome(client, accessToken, refreshToken, { name, description, thumbnail, photoUploadVector }, { outcomes, models } = {}) {
  ...
  const qs = query.toString();
  if (qs) path += '?' + qs;
  return client.request(path, 'POST', accessToken, refreshToken, { name, description, thumbnail, photo_upload_vector: photoUploadVector });
}

export function updateProjects(client, accessToken, refreshToken, projectId, body = {}) {
  const path = `/api/v1/projects/${encodeURIComponent(projectId)}`;
...
export function createProjectsEvent(client, accessToken, refreshToken, projectTableName, { event, detail }) {
  const path = `/api/v1/projects/${encodeURIComponent(projectTableName)}/event`;
  return client.request(path, 'POST', accessToken, refreshToken, { event, detail });
}
