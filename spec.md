...
- Honoring per-field `required` metadata on flat-dict request bodies: PHP/Python give optional fields `null`/`None` defaults (declaration order is kept, so the YAML must declare required fields first — a required field after an optional one fails generation via `_flat_body_fields`); JS takes the whole body as one destructured object. Bodies with optional fields are emitted through a null-dropping wrapper (`array_filter(..., fn($v) => $v !== null)` in PHP, a `v is not None` dict comprehension in Python, `JSON.stringify`'s native undefined-dropping in JS) so absent optionals never reach the wire — the photo-upload endpoint, whose metadata moved from `x-*` headers into the body, is the canonical consumer
- **Positional flat-dict style (`positional: true` opt-in):** a flat-dict field may carry `positional: true` to opt the whole endpoint into a positional signature (`_flat_dict_positional` gate + `_flat_body_categories` split in `base.py`). Required fields become positional args (no default); optional fields marked `positional: true` become positional args with a default; every remaining optional collapses into a trailing named options bag — a destructured `{ … } = {}` in JS, keyword-only params after `*` in Python, an `array $options = []` in PHP (PHP body-build reads those fields as `$options['camelKey'] ?? null`). Category grouping (required → positional-optional → bag) guarantees required-before-optional ordering regardless of YAML field order. Un-marked flat-dict bodies keep the legacy signature unchanged, so the change is scoped to opted-in endpoints. The photo-upload endpoint (`upload_photo_to_mediaviz`) marks its model-toggle fields (`blur`, `colors`, `face_recognition`, `image_classification`, `image_comparison`, `image_describe`, `ocr`) positional; its composite `upload_photo` is unchanged (JS's same-controller sibling call adapts to the new positional order automatically; PHP/Python composites inline the HTTP call and are already decoupled from the method signature)
...
**Declarations.** `dist/sdk.d.ts` / `.esm.d.ts` / `.d.cts` carry the catalog-derived endpoint surface exactly as for the browser SDK, plus a hand-written addendum declaring the adapters copied in from `react_native_module/` — `createCryptoProvider`, `configureCrypto`, the three token stores, `startAuthSession`, `parseRedirectUrl`, `AuthSessionError`, `createSession` and their supporting types. These are re-exported from the package root by the barrel, so the package root is where TypeScript looks for them; without the addendum they shipped as untyped runtime exports and a consumer got TS2305 on the very APIs the framework exists to add. `JavaScriptBrowserGenerator.dts_addendum()` returns `""`, so the browser SDK is unaffected. A `dist/react.d.ts` / `.d.cts` covers the subpath separately. Both hand-written surfaces are the fixed shape of the bundled source, not something the endpoint catalog can vary, and a test asserts the addendum declares every name the barrel exports so the two cannot drift. All are type-checked with `tsc --strict`.
...
- **Method params** mirror the JS argument order (`path → required headers → body → optional-header bag → query options bag`); request params are camelCase. Expanded request bodies with required fields become positional params (required first, then `?`-optional); all-optional bodies collapse to one `body?: { … }` options bag, matching `_js_body_sig_tokens`. A flat-dict body in the positional style (`positional: true` opt-in) emits required/positional-optional fields as positional params (`?`-optional for the latter) followed by an `options?: { … }` bag for the rest; un-marked flat-dict bodies stay a single `body: { … }` object. Reserved-word params are escaped (`private` → `private_`) via the shared `_js_safe`.
...
| `steps[].input_map` | Maps endpoint params or utility params to dot-path expressions: `params.*`, `params.<obj>.<prop>`, `steps.<id>.*`. For utility steps, the keys are utility param names (canonical snake_case from the utility YAML) and the call is positional in declaration order. Field access on a prior step's output (`steps.<id>.<field>`) is emitted **null-safe** in every generator — `template?.field` (JS), `($template['field'] ?? null)` (PHP, via the optional-header guard), `(template or {}).get('field')` (Python) — since step outputs are runtime responses that may omit fields; caller-supplied `params.*` keep strict access. A fourth form, `model_flag:<step>:<field>`, reads a boolean model toggle out of a step output's `body` map (`<step>.body.<field>`), falling back to the legacy `headers["x-<field-with-hyphens>"]` spelling for templates that predate the header→body upload migration, and coerces it to the string `"true"` (emitted only when enabled; omitted otherwise). This exists because the plain dot-path syntax cannot express two-location fallback with boolean→string coercion — needed by the upload composite, whose template returns model flags under `template.body.blur` as booleans (legacy: `template.headers["x-blur"]`). |
