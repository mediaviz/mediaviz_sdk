...
class Photos {
    ...
    public function getPhotosByMonthYearSort(
        ...
        $path = "/api/v1/photos/" . urlencode($projectTable) . "/month/" . urlencode($month) . "/year/" . urlencode($year) . "/";
        $query = [];
        if ($ascOrDesc !== null) $query['asc_or_desc'] = $ascOrDesc;
        if ($lastId !== null) $query['last_id'] = $lastId;
        if ($limit !== null) $query['limit'] = $limit;
    ...
    public function getPhotosRanked(
        ...
        $path = "/api/v1/photos_ranked/" . urlencode($tableName) . "/";
        $query = [];
        if ($ascOrDesc !== null) $query['asc_or_desc'] = $ascOrDesc;
        if ($lastId !== null) $query['last_id'] = $lastId;
        if ($limit !== null) $query['limit'] = $limit;
    ...
    public function getPhotosMonthYearRanked(
        ...
        $path = "/api/v1/photos/" . urlencode($projectTable) . "/month/" . urlencode($month) . "/year/" . urlencode($year) . "/ranked";
        $query = [];
        if ($ascOrDesc !== null) $query['asc_or_desc'] = $ascOrDesc;
        if ($query) $path .= '?' . http_build_query($query);
        return $this->client->request($path, 'GET', $accessToken, $refreshToken);
    ...
    public function getPhotosDateTakenNoneRankedSortDesc(
        ...
        $path = "/api/v1/photos/" . urlencode($projectTable) . "/date_taken/none/ranked";
        $query = [];
        if ($ascOrDesc !== null) $query['asc_or_desc'] = $ascOrDesc;
        if ($query) $path .= '?' . http_build_query($query);
        return $this->client->request($path, 'GET', $accessToken, $refreshToken);
