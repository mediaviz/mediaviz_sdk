...
class OAuthClientTest extends TestCase
{
    ...
    public function test_request_returns_data_with_null_updated_tokens_on_2xx(): void
    {
        ...
        $result = $client->request('https://api.example.com/me', 'GET', 'access-tok', 'refresh-tok');
    ...
    {
        $capturedHeaders = [];
        $transport = function (string $url, string $method, array $headers, ?string $body) use (&$capturedHeaders): array {
            $capturedHeaders = $headers;
            return [200, json_encode(['ok' => true])];
        };
        $client = new OAuthClient($this->config, $transport);
        $client->request('https://api.example.com/me', 'GET', 'my-token', 'refresh-tok');
    ...
    public function test_request_returns_data_and_updated_tokens_on_401_refresh_retry_success(): void
    {
        ...
        $client = new OAuthClient($this->config, $transport);
        $result = $client->request('https://api.example.com/me', 'GET', 'old-access', 'refresh-tok');
    ...
    public function test_request_sends_body_as_json_with_content_type_header(): void
    {
        ...
        $client = new OAuthClient($this->config, $transport);
        $client->request('https://api.example.com/items', 'POST', 'access-tok', 'refresh-tok', ['name' => 'test']);
