...
class OAuthClient {
  ...
  async request(url, method, accessToken, refreshToken, body) {
    ...
    const firstResponse = await fetch(url, buildOptions(accessToken));
    ...
    const retryResponse = await fetch(url, buildOptions(newTokens.access_token));
  ...
  decodeAccessToken(accessToken) {
    ...
    const padded = segment + '=='.slice((segment.length % 4) || 4);
