...
class Photoupload {
    ...
    public function uploadPhoto(
        ...
        if (isset($this->_get_templateCache[$_cacheKey])) {
            ...
            $template = $this->client->request($_path, 'GET', $accessToken, $refreshToken)->data;
        ...
        if ($photo['client_side_id'] !== null) $_headers[] = 'x-client-side-id: ' . $photo['client_side_id'];
        if ($photo['blur'] !== null) $_headers[] = 'x-blur: ' . $photo['blur'];
        if ($photo['colors'] !== null) $_headers[] = 'x-colors: ' . $photo['colors'];
        if ($photo['face_recognition'] !== null) $_headers[] = 'x-face-recognition: ' . $photo['face_recognition'];
        if ($photo['image_classification'] !== null) $_headers[] = 'x-image-classification: ' . $photo['image_classification'];
        if ($photo['image_comparison'] !== null) $_headers[] = 'x-image-comparison: ' . $photo['image_comparison'];
        if ($photo['size'] !== null) $_headers[] = 'x-size: ' . $photo['size'];
        if ($photo['source_resolution_x'] !== null) $_headers[] = 'x-source-resolution-x: ' . $photo['source_resolution_x'];
        if ($photo['source_resolution_y'] !== null) $_headers[] = 'x-source-resolution-y: ' . $photo['source_resolution_y'];
        if ($photo['date_taken'] !== null) $_headers[] = 'x-date-taken: ' . $photo['date_taken'];
        if ($photo['latitude'] !== null) $_headers[] = 'x-latitude: ' . $photo['latitude'];
        if ($photo['longitude'] !== null) $_headers[] = 'x-longitude: ' . $photo['longitude'];
        if ($photo['resized_dimensions'] !== null) $_headers[] = 'x-resized-dimensions: ' . $photo['resized_dimensions'];
        $_body = [
        ...
        $_ch = curl_init();
        curl_setopt($_ch, CURLOPT_URL, $baseUrl . "/photo_upload");
