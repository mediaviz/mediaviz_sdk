...
class Admin {
  ...
  async adminResyncS3BucketAllowlists() {
    this._ctx.requireTokens();
    const path = `/api/v1/admin/resync_s3_bucket_allowlists`;
    const { data } = await this._ctx.client.request(path, 'POST', this._ctx.accessToken, this._ctx.refreshToken);
    return data;
  }
...
class Company {
  ...
  async updateCompanyPhotoUrlAllowlist(companyId, photoUrlAllowlist) {
    this._ctx.requireTokens();
    const path = `/api/v1/company/${encodeURIComponent(companyId)}/photo_url_allowlist/`;
...
class Search {
  ...
  async searchProjectPhotosNaturalLanguage(projectTableName, searchText, { minCosine, labelMinCosine, labelTopK, labelDelta, bestOfSimilarSetsOnly, splitByTier } = {}) {
    this._ctx.requireTokens();
...
class Users {
  ...
  async createUserAndCompany(name, email, password, companyId = undefined, profilePicture = undefined, paymentPlanType = undefined, companyName = undefined, credits = undefined, photoUrlAllowlist = undefined, { inviteToken } = {}) {
