...
class Admin {
  ...
  async adminCreateNlHybridPipelines() {
    this._ctx.requireTokens();
    const path = `/api/v1/admin/search/nl_create_hybrid_pipelines`;
    const { data } = await this._ctx.client.request(path, 'POST', this._ctx.accessToken, this._ctx.refreshToken);
    return data;
  }
...
class Search {
  ...
  async searchProjectPhotosNaturalLanguageAuto(projectTableName, searchText, size = undefined, { blend, minCosine, labelMinCosine, labelTopK, labelDelta, debugScores } = {}) {
    this._ctx.requireTokens();
