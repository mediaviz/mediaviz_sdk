...
describe('Search', () => {
  ...
  it('searchProjectPhotosNaturalLanguageAuto — HTTP method is POST', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const search = new Search(ctx);
    await search.searchProjectPhotosNaturalLanguageAuto('test_value', 'test_value', 42, { blend: 'test_value', minCosine: 3.14, labelMinCosine: 3.14, labelTopK: 42, labelDelta: 3.14 });
    expect(spy.last_call().method).toBe('POST');
  });
  ...
  it('searchProjectPhotosNaturalLanguageAuto — path construction', async () => {
    ...
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const search = new Search(ctx);
    await search.searchProjectPhotosNaturalLanguageAuto('hello world', 'test_value', 42, { blend: 'test_value', minCosine: 3.14, labelMinCosine: 3.14, labelTopK: 42, labelDelta: 3.14 });
