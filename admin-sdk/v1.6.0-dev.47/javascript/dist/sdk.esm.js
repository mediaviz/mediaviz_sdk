...
function requireCrypto () {
	...
	crypto = { configureCrypto, resetCrypto, getCrypto };
	return crypto;
}
...
function requireSrc$1 () {
	...
	const { configureCrypto } = requireCrypto();

	src$1 = { OAuthClient, OAuthError, OAuthErrorCode, configureCrypto };
	return src$1;
}
...
class Admin {
  ...
  async adminBackfillFilePathField() {
    this._ctx.requireTokens();
    const path = `/api/v1/admin/search/backfill_file_path_field`;
    const { data } = await this._ctx.client.request(path, 'POST', this._ctx.accessToken, this._ctx.refreshToken);
    return data;
  }

  async adminBackfillColorFamiliesField() {
    this._ctx.requireTokens();
    const path = `/api/v1/admin/search/backfill_color_families_field`;
    const { data } = await this._ctx.client.request(path, 'POST', this._ctx.accessToken, this._ctx.refreshToken);
    return data;
  }
...
class Company {
  ...
  async adminGetCompanies() {
    this._ctx.requireTokens();
    const path = `/api/v1/companies/admin`;
    const { data } = await this._ctx.client.request(path, 'GET', this._ctx.accessToken, this._ctx.refreshToken);
    return data;
  }
...
function stripUndef$9(o) { const r = {}; for (const k in o) if (o[k] !== undefined) r[k] = o[k]; return r; }
...
class Keywords {
  ...
  async adminAddProjectsToKeywordFilteringList(keywordListId, { projectIds } = {}) {
    this._ctx.requireTokens();
    let path = `/api/v1/keyword/admin/${encodeURIComponent(keywordListId)}/projects`;
    const query = new URLSearchParams();
    if (projectIds !== undefined) (Array.isArray(projectIds) ? projectIds : [projectIds]).forEach(v => query.append('project_ids', v));
    const qs = query.toString();
    if (qs) path += '?' + qs;
  ...
  async adminAddCompanyToKeywordFilteringList(keywordListId, { companyId } = {}) {
    this._ctx.requireTokens();
    let path = `/api/v1/keyword/admin/${encodeURIComponent(keywordListId)}/company`;
    const query = new URLSearchParams();
    if (companyId !== undefined) (Array.isArray(companyId) ? companyId : [companyId]).forEach(v => query.append('company_id', v));
    const qs = query.toString();
    if (qs) path += '?' + qs;
  ...
  async adminRemoveProjectsFromKeywordFilteringList(keywordListId, { projectIds } = {}) {
    this._ctx.requireTokens();
    let path = `/api/v1/keyword/admin/${encodeURIComponent(keywordListId)}/projects`;
    const query = new URLSearchParams();
    if (projectIds !== undefined) (Array.isArray(projectIds) ? projectIds : [projectIds]).forEach(v => query.append('project_ids', v));
  ...
  async adminRemoveCompanyFromKeywordFilteringList(keywordListId, { companyId } = {}) {
    this._ctx.requireTokens();
    let path = `/api/v1/keyword/admin/${encodeURIComponent(keywordListId)}/company`;
    const query = new URLSearchParams();
    if (companyId !== undefined) (Array.isArray(companyId) ? companyId : [companyId]).forEach(v => query.append('company_id', v));
...
export { Admin, AiModelCredits, ApiError, Company, CuratedAlbums, CustomAlbums, EmailTokens, Health, InMemoryWebhookStore, Keywords, MediaViz, NotFoundError, OAuthClient, OAuthError, OAuthErrorCode, OauthAuthorization, OauthClients, OauthLogin, OauthToken, Person, Photos, Photoupload, Projects, RateLimitError, Search, ServerError, Subscription, Users, ValidationError, WEBHOOK_MODEL_COLUMNS, WEBHOOK_OUTCOME_MODELS, WebhookConsumer, Webhookreceiver, configureCrypto, handleResponse, selectResultColumns, signWebhookPayload, verifyWebhookSignature };
