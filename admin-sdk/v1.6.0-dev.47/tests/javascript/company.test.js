...
describe('Company', () => {
  it('adminGetCompanies — exists', () => {
    const company = new Company({});
    expect(typeof company.adminGetCompanies).toBe('function');
  });

  it('adminGetCompanies — HTTP method is GET', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const company = new Company(ctx);
    await company.adminGetCompanies();
    expect(spy.last_call().method).toBe('GET');
  });

  it('adminGetCompanies — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const company = new Company(ctx);
    await company.adminGetCompanies();
    expect(spy.last_call().path).toContain('/api/v1/companies/admin');
  });

  it('adminGetCompanies — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const company = new Company(ctx);
    await company.adminGetCompanies();
