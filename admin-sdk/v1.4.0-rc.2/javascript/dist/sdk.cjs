...
class Photoupload {
  ...
  async uploadPhoto(projectTableName, companyId, userId, photoIndex, photo) {
    ...
    const upload_result = await this.uploadPhotoToMediaviz(companyId, userId, projectTableName, photo.title, { fileContent: photo.fileContent, mimetype: photo.mimetype, filePath: photo.filePath }, { clientSideId: photo.clientSideId, blur: template?.blur, colors: template?.colors, faceRecognition: template?.face_recognition, imageDescribe: template?.image_describe, imageClassification: template?.image_classification, imageComparison: template?.image_comparison, size: photo.size, sourceResolutionX: photo.sourceResolutionX, sourceResolutionY: photo.sourceResolutionY, dateTaken: photo.dateTaken, latitude: photo.latitude, longitude: photo.longitude, ocr: template?.ocr });
