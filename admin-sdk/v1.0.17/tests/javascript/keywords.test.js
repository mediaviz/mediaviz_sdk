...
describe('Keywords', () => {
  ...
  it('adminAddProjectsToKeywordFilteringList — exists', () => {
    const keywords = new Keywords({});
    expect(typeof keywords.adminAddProjectsToKeywordFilteringList).toBe('function');
  });

  it('adminAddProjectsToKeywordFilteringList — HTTP method is POST', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminAddProjectsToKeywordFilteringList(42, { projectIds: 'test_value' });
    expect(spy.last_call().method).toBe('POST');
  });

  it('adminAddProjectsToKeywordFilteringList — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminAddProjectsToKeywordFilteringList(42, { projectIds: 'test_value' });
    expect(spy.last_call().path).toContain('/api/v1/keyword/admin/42/projects');
  });

  it('adminAddProjectsToKeywordFilteringList — query params', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminAddProjectsToKeywordFilteringList(42, { projectIds: 'test_value' });
  ...
  it('adminAddProjectsToKeywordFilteringList — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminAddProjectsToKeywordFilteringList(42, { projectIds: 'test_value' });
  ...
  it('addCompanyToKeywordFilteringList — exists', () => {
    const keywords = new Keywords({});
    expect(typeof keywords.addCompanyToKeywordFilteringList).toBe('function');
  });

  it('addCompanyToKeywordFilteringList — HTTP method is POST', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.addCompanyToKeywordFilteringList(42, { companyId: 42 });
    expect(spy.last_call().method).toBe('POST');
  });

  it('addCompanyToKeywordFilteringList — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.addCompanyToKeywordFilteringList(42, { companyId: 42 });
    expect(spy.last_call().path).toContain('/api/v1/keyword/42/company');
  });

  it('addCompanyToKeywordFilteringList — query params', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.addCompanyToKeywordFilteringList(42, { companyId: 42 });
  ...
  it('addCompanyToKeywordFilteringList — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.addCompanyToKeywordFilteringList(42, { companyId: 42 });
  ...
  it('adminAddCompanyToKeywordFilteringList — exists', () => {
    const keywords = new Keywords({});
    expect(typeof keywords.adminAddCompanyToKeywordFilteringList).toBe('function');
  });

  it('adminAddCompanyToKeywordFilteringList — HTTP method is POST', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminAddCompanyToKeywordFilteringList(42, { companyId: 42 });
    expect(spy.last_call().method).toBe('POST');
  });

  it('adminAddCompanyToKeywordFilteringList — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminAddCompanyToKeywordFilteringList(42, { companyId: 42 });
    expect(spy.last_call().path).toContain('/api/v1/keyword/admin/42/company');
  });

  it('adminAddCompanyToKeywordFilteringList — query params', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminAddCompanyToKeywordFilteringList(42, { companyId: 42 });
  ...
  it('adminAddCompanyToKeywordFilteringList — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminAddCompanyToKeywordFilteringList(42, { companyId: 42 });
  ...
  it('adminRemoveProjectsFromKeywordFilteringList — exists', () => {
    const keywords = new Keywords({});
    expect(typeof keywords.adminRemoveProjectsFromKeywordFilteringList).toBe('function');
  });

  it('adminRemoveProjectsFromKeywordFilteringList — HTTP method is DELETE', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminRemoveProjectsFromKeywordFilteringList(42, { projectIds: 'test_value' });
    expect(spy.last_call().method).toBe('DELETE');
  });

  it('adminRemoveProjectsFromKeywordFilteringList — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminRemoveProjectsFromKeywordFilteringList(42, { projectIds: 'test_value' });
  ...
  it('adminRemoveProjectsFromKeywordFilteringList — query params', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminRemoveProjectsFromKeywordFilteringList(42, { projectIds: 'test_value' });
  ...
  it('adminRemoveProjectsFromKeywordFilteringList — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminRemoveProjectsFromKeywordFilteringList(42, { projectIds: 'test_value' });
  ...
  it('removeCompanyFromKeywordFilteringList — exists', () => {
    const keywords = new Keywords({});
    expect(typeof keywords.removeCompanyFromKeywordFilteringList).toBe('function');
  });

  it('removeCompanyFromKeywordFilteringList — HTTP method is DELETE', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.removeCompanyFromKeywordFilteringList(42, { companyId: 42 });
    expect(spy.last_call().method).toBe('DELETE');
  });

  it('removeCompanyFromKeywordFilteringList — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.removeCompanyFromKeywordFilteringList(42, { companyId: 42 });
  ...
  it('removeCompanyFromKeywordFilteringList — query params', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.removeCompanyFromKeywordFilteringList(42, { companyId: 42 });
  ...
  it('removeCompanyFromKeywordFilteringList — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.removeCompanyFromKeywordFilteringList(42, { companyId: 42 });
  ...
  it('adminRemoveCompanyFromKeywordFilteringList — exists', () => {
    const keywords = new Keywords({});
    expect(typeof keywords.adminRemoveCompanyFromKeywordFilteringList).toBe('function');
  });

  it('adminRemoveCompanyFromKeywordFilteringList — HTTP method is DELETE', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminRemoveCompanyFromKeywordFilteringList(42, { companyId: 42 });
    expect(spy.last_call().method).toBe('DELETE');
  });

  it('adminRemoveCompanyFromKeywordFilteringList — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminRemoveCompanyFromKeywordFilteringList(42, { companyId: 42 });
  ...
  it('adminRemoveCompanyFromKeywordFilteringList — query params', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminRemoveCompanyFromKeywordFilteringList(42, { companyId: 42 });
  ...
  it('adminRemoveCompanyFromKeywordFilteringList — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const keywords = new Keywords(ctx);
    await keywords.adminRemoveCompanyFromKeywordFilteringList(42, { companyId: 42 });
