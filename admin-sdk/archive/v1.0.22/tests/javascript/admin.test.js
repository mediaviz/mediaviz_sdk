...
describe('Admin', () => {
  ...
  it('adminCreateLabelVocabIndex — exists', () => {
    const admin = new Admin({});
    expect(typeof admin.adminCreateLabelVocabIndex).toBe('function');
  });

  it('adminCreateLabelVocabIndex — HTTP method is POST', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminCreateLabelVocabIndex();
    expect(spy.last_call().method).toBe('POST');
  });

  it('adminCreateLabelVocabIndex — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminCreateLabelVocabIndex();
    expect(spy.last_call().path).toContain('/api/v1/admin/search/create_label_vocab_index');
  });

  it('adminCreateLabelVocabIndex — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminCreateLabelVocabIndex();
