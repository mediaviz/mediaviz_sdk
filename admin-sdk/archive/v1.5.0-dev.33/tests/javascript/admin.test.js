...
describe('Admin', () => {
  ...
  it('adminBackfillFilePathField — exists', () => {
    const admin = new Admin({});
    expect(typeof admin.adminBackfillFilePathField).toBe('function');
  });

  it('adminBackfillFilePathField — HTTP method is POST', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminBackfillFilePathField();
    expect(spy.last_call().method).toBe('POST');
  });

  it('adminBackfillFilePathField — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminBackfillFilePathField();
    expect(spy.last_call().path).toContain('/api/v1/admin/search/backfill_file_path_field');
  });

  it('adminBackfillFilePathField — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminBackfillFilePathField();
  ...
  it('adminBackfillColorFamiliesField — exists', () => {
    const admin = new Admin({});
    expect(typeof admin.adminBackfillColorFamiliesField).toBe('function');
  });

  it('adminBackfillColorFamiliesField — HTTP method is POST', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminBackfillColorFamiliesField();
    expect(spy.last_call().method).toBe('POST');
  });

  it('adminBackfillColorFamiliesField — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminBackfillColorFamiliesField();
    expect(spy.last_call().path).toContain('/api/v1/admin/search/backfill_color_families_field');
  });

  it('adminBackfillColorFamiliesField — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminBackfillColorFamiliesField();
  ...
  it('adminResyncS3BucketAllowlists — exists', () => {
    const admin = new Admin({});
    expect(typeof admin.adminResyncS3BucketAllowlists).toBe('function');
  });

  it('adminResyncS3BucketAllowlists — HTTP method is POST', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminResyncS3BucketAllowlists();
    expect(spy.last_call().method).toBe('POST');
  });

  it('adminResyncS3BucketAllowlists — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminResyncS3BucketAllowlists();
    expect(spy.last_call().path).toContain('/api/v1/admin/resync_s3_bucket_allowlists');
  });

  it('adminResyncS3BucketAllowlists — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminResyncS3BucketAllowlists();
  ...
  it('adminClearNlSearchCache — HTTP method is DELETE', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminClearNlSearchCache();
    expect(spy.last_call().method).toBe('DELETE');
  });

  it('adminClearNlSearchCache — path construction', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminClearNlSearchCache();
    expect(spy.last_call().path).toContain('/api/v1/admin/clear_nl_search_cache/');
  });

  it('adminClearNlSearchCache — auth routing', async () => {
    const spy = new SpyOAuthClient();
    const ctx = { client: spy, accessToken: 'access_token', refreshToken: 'refresh_token', requireTokens: () => {} };
    const admin = new Admin(ctx);
    await admin.adminClearNlSearchCache();
